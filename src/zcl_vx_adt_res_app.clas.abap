class ZCL_VX_ADT_RES_APP definition
  public
...
CLASS ZCL_VX_ADT_RES_APP IMPLEMENTATION.


  METHOD fill_router.
    " Every VERTEX service registers here rather than under a prefix of its
    " own: the BAdI filter already claims /sap/bc/adt/vertex/*, and a second
    " prefix would mean a second implementation and a second filter to get
    " wrong.
    " The routes are the list ZCL_VX_ADT_RES_ABOUT reports, so what a window
    " is told this system has and what the router serves cannot drift apart.
    DATA(lt_service) = zcl_vx_adt_res_about=>services( ).
