CLASS zcl_vx_tools DEFINITION PUBLIC CREATE PUBLIC.
  PUBLIC SECTION.
    TYPES: BEGIN OF t_pair,
           ...
           tt_jfld TYPE zif_vx_pivot_types=>tt_jfld, "shared with zcl_vx_pivot, see zif_vx_pivot_types
    ...
    METHODS: constructor IMPORTING i_tabname TYPE tabname,
      ...
      set_pivot    IMPORTING it_rows TYPE zcl_vx_pivot=>tt_keys
                             it_cols TYPE zcl_vx_pivot=>tt_keys
                             it_vals TYPE zcl_vx_pivot=>tt_vals,
  ...
  PROTECTED SECTION.
    " APPLY_LAYOUT reads this; it stays unbound without a window, and the CHECK
    " there then does nothing. A subclass that builds a window fills it.
    DATA mo_splitter TYPE REF TO cl_gui_splitter_container.

    " Where a finished result goes. Here: nowhere - the caller already has it
    " in ER_RESULT. A subclass with a window overrides this and draws it.
    METHODS on_result IMPORTING ir_tab     TYPE REF TO data
                                i_name     TYPE string
  ...
  PRIVATE SECTION.
    ...
    DATA: m_tabname    TYPE tabname,
          ...
          mo_pivot     TYPE REF TO zcl_vx_pivot,
          ...
          mt_where_sel TYPE TABLE OF zcl_vx_appl=>selection_display_s, "ranges before pivot rebind
    ...
    METHODS:
      ...
      value_label IMPORTING i_row          TYPE any
                            it_comps       TYPE string_table
                            it_cols        TYPE zcl_vx_pivot=>tt_sqlcols
                  RETURNING VALUE(rv_text) TYPE string,
...
CLASS zcl_vx_TOOLS IMPLEMENTATION.


  METHOD on_result.
    " Nothing to draw into. See the declaration.
    RETURN.
  ENDMETHOD.
  ...
  METHOD constructor.
    ...
    " Everything a caller that renders elsewhere needs: the candidates and the
    " join model. M_READY stays false, so nothing later tries to push a result
    " into a window that is not there, and the filters arrive through
    " SET_FILTERS rather than from a selection panel.
    find_candidates( ).
    rebuild_selection( ).
  ENDMETHOD.
  ...
  METHOD open_layout_for.
    " In SDE a layout saved for another table opens a second window for that
    " table. There is no window to open here, so the load stops and says which
    " table the file actually belongs to - loading it against the wrong base
    " would silently produce a join of the wrong thing.
    MESSAGE |Layout { i_file } belongs to { i_base }, not { m_tabname }| TYPE 'S' DISPLAY LIKE 'E'.
  ENDMETHOD.
  ...
  METHOD discover_for.
    ...
    zcl_vx_ddic=>get_text_table( EXPORTING i_tname = i_tabname IMPORTING e_tab = DATA(l_texttab) ).
  ...
  METHOD add_candidate.
    ...
    IF zcl_vx_sql=>exist_table( l_tabname ) NE 1 AND zcl_vx_sql=>exist_view( l_tabname ) NE 1.
  ...
  METHOD render_html.
    ...
    LOOP AT zcl_vx_appl=>mt_lang INTO DATA(ls_top_lang).
  ...
  METHOD cfg_dir.
    ...
    rv_dir = condense( zcl_vx_appl=>gv_path ).
    ...
    IF m_mode = 'P' AND mo_pivot IS BOUND AND mo_pivot->has_layout( ) = abap_true.
      ...
      rv_sql = mo_pivot->build_select( i_from  = build_from( )
                                       ...
                                       i_rows  = COND #( WHEN i_rows > 0 THEN i_rows
                                                         WHEN zcl_vx_appl=>gv_rows > 0
                                                         THEN zcl_vx_appl=>gv_rows
                                                         ELSE 500 ) ).
    ELSE.
  ...
  METHOD load_config.
    ...
    DATA: lt_stabs TYPE TABLE OF t_saved_tab,
          ...
          lt_wsel  TYPE TABLE OF zcl_vx_appl=>selection_display_s,
          lt_prows TYPE zcl_vx_pivot=>tt_keys,
          lt_pcols TYPE zcl_vx_pivot=>tt_keys,
          lt_pvals TYPE zcl_vx_pivot=>tt_vals,
    ...
    " filters: into the cache. SDE also writes them straight into the panel
    " rows, because a refresh during the rebuild would re-cache them as empty;
    " with no panel the cache is the whole story.
  ...
  METHOD fill_sel_extras.
    " Puts the joined tables' extra fields on the selection panel of the
    " window. No panel here; a subclass that has one overrides it.
    RETURN.
  ENDMETHOD.
  ...
  METHOD update_sql_view.
    ...
    IF m_mode = 'P' AND mo_pivot IS BOUND AND mo_pivot->has_layout( ) = abap_true.
      ...
      l_sql = mo_pivot->build_select( i_from  = build_from( )
                                      i_where = build_where( )
                                      i_multi = is_multi( )
                                      i_rows  = COND #( WHEN zcl_vx_appl=>gv_rows > 0 THEN zcl_vx_appl=>gv_rows ELSE 500 ) ).
    ELSE.
  ...
  METHOD viewer_alive.
    " Nothing to be alive: this one was built without a window.
    rv_alive = abap_false.
  ENDMETHOD.
  ...
  METHOD generate_select.
    ...
    DATA(l_rows) = COND i( WHEN zcl_vx_appl=>gv_rows > 0 THEN zcl_vx_appl=>gv_rows ELSE 500 ).
  ...
  METHOD cache_where_selection.
    " Reads the selection panel into the filter cache. There is no panel here;
    " SET_FILTERS fills the cache instead. A subclass with a window overrides it.
    RETURN.
  ENDMETHOD.
  ...
  METHOD build_where.
    DATA: lt_where TYPE rsds_twhere,
          ...
          lt_sel   TYPE TABLE OF zcl_vx_appl=>selection_display_s.
  ...
  METHOD execute_sql.
    ...
    l_rows = COND i( WHEN zcl_vx_appl=>gv_rows > 0 THEN zcl_vx_appl=>gv_rows ELSE 500 ).
    ...
    " No window here: the caller has the rows in ER_RESULT. A subclass that
    " has one overrides ON_RESULT and draws it there.
    DATA(l_view_name) = COND #( WHEN l_group IS INITIAL
                                THEN |JOIN { m_tabname } ({ lines( <result> ) })|
                                ELSE |PIVOT { m_tabname } ({ lines( <result> ) })| ).
    on_result( ir_tab     = lr_table
               i_name     = l_view_name
               it_catalog = lt_cat ).
    sync_sel_panel( ).
  ENDMETHOD.
  ...
  METHOD sync_sel_panel.
    " Writes the filters back into the selection panel of the window this was
    " opened from. There is no panel here; a subclass that has one overrides it.
    RETURN.
  ENDMETHOD.
  ...
  METHOD execute_pivot.
    ...
    DATA: lt_ncomp  TYPE abap_component_tab,
          ...
          lt_meas   TYPE zcl_vx_pivot=>tt_sqlcols,
    ...
    DATA(l_rows)  = COND i( WHEN i_rows > 0                 THEN i_rows
                            WHEN zcl_vx_appl=>gv_rows > 0  THEN zcl_vx_appl=>gv_rows
                            ELSE 500 ).
    ...
    " No window here: the caller has the matrix in ER_RESULT. A subclass that
    " has one overrides ON_RESULT and draws it there.
    on_result( ir_tab     = lr_wide
